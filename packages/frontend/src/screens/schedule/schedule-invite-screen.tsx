// =============================================================
// 外部ゲスト向け共有招待URL回答画面（認証不要・公開ページ）
// /schedule/invite/:inviteToken
// 1つのURLを複数人で共有できる方式。回答時に新規ゲストを作成する。
// =============================================================
...
export function ScheduleInviteScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/schedule/invite/${inviteToken}`)
      .then((r) => r.json())
      .then((json) => {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(`${API_BASE_URL}/schedule/invite/${inviteToken}/respond`, {
        method: "POST",
      ...
      setPersonalUrl(json.scheduleUrl ?? "");
      setSubmitted(true);
    } catch {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: "var(--color-paper-100)" }}>
        ...
        <p className="text-sm mt-2" style={{ color: "var(--color-ink-400)" }}>URLが無効か、既に締め切られています</p>
      </div>
    );
  }
          ...
          <div className="mt-6 card-paper rounded-3xl p-5 max-w-sm w-full text-left space-y-3">
            ...
            <div className="border-t pt-3" style={{ borderColor: "var(--color-paper-300)" }}>
              <p className="text-xs font-medium mb-2" style={{ color: "var(--color-ink-600)" }}>
                📌 あなた専用のページURL（後から回答変更・確定確認に使えます）
              </p>
              <div className="flex items-center gap-2 p-2 rounded-xl" style={{ background: "var(--color-paper-200)" }}>
                <p className="flex-1 text-xs break-all" style={{ color: "var(--color-ink-500)" }}>{personalUrl}</p>
                <button
                  onClick={handleCopy}
                  ...
                  <div key={cand.id} className="card-paper rounded-2xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div className="flex gap-2 shrink-0">
                      {AVAIL_OPTIONS.map((o) => (
                        <button
                          key={o.value}
                          onClick={() => setAnswer(cand.id, o.value)}
