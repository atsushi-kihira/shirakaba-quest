...
function formatDate(ts: number, endsAt: number | null): string {
  ...
  if (endsAt) {
    ...
    return `${date} ${startT}〜${e.getHours().toString().padStart(2,"0")}:${e.getMinutes().toString().padStart(2,"0")}`;
  }
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: "var(--color-paper-100)" }}>
        ...
        <div className="mt-6 card-paper rounded-3xl p-4 max-w-sm w-full text-left">
                  ...
                  <div key={cand.id} className="card-paper rounded-2xl px-4 py-3 flex items-center gap-3">
                    <div className="flex-1">
                      <p className="text-sm font-medium" style={{ color: "var(--color-ink-800)" }}>
                        {formatDate(cand.startsAt, cand.endsAt)}
                      </p>
